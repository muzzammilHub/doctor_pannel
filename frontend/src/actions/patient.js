...
export const loadPatient = ()=> async(dispatch)=>{
    try {
        const {data} = await axios.get('https://backend-app-n7as.onrender.com/api/v1/allPatient',{
            headers:{
...
export const loadMedicalInfo = (id)=> async(dispatch)=>{
    try {
        const {data} = await axios.get(`https://backend-app-n7as.onrender.com/api/v1/getHistory?id=${id}`,{
            headers:{
                Authorization: `Bearer ${localStorage.getItem('authToken')}`
            }
        })
